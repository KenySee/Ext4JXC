Ext.define('Keer.ui.core1.PartWork.MainController',{
	extend: 'Keer.widget.mvc.Controller',
	//【加载依赖】
	requires: [
		'Keer.ui.core1.PartWork.EditWindow'
	],
	//【混入功能】
	mixins: {
	},
	...
	onBeforeLoad: function(store,params){
  	...
  	onCreateRecord: function(component,record){
     	...
     	if (component == grid){
	  		var navModel = this.getNavModel();
	   		if (navModel){
	   			record.set('artist',this.getJSONBean(navModel,'nickname'));
	   		}
	   	}
  	},
  	//【navStore加载前执行】
	...
	doEdit: function(){
		...
		if (model){
			this.doOpenEditWindow('ui-core1-PartWork-editwindow',grid,store,model,false);
		}
		else {
	...
	doAddRecord: function(store,model){
		var grid = this.getGridView();
		this.doOpenEditWindow('ui-core1-PartWork-editwindow',grid,store,model,true);
	},
	doSave: function(){
		var store = this.getGridStore();
