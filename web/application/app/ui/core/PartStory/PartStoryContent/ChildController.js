Ext.define('Keer.ui.core.PartStory.PartStoryContent.ChildController',{
	...
	requires: [
		...
		'Keer.ui.core.PartStory.PartStoryContent.EditWindow'
	],
	//【混入功能】
	mixins: {
	},
	...
	onBeforeLoad: function(store,params){
  	...
  	onCreateRecord: function(component,record){
     	...
     	var parent = this.get('parent');
		if (parent){
			record.set('story',this.getJSONBean(parent,'name'));
		}
  	},
	...
	doEdit: function(){
		...
		if (model){
			this.doOpenEditWindow('ui-core-PartStory-PartStoryContent-editwindow',grid,store,model,false);
		}
		else {
	...
	doAddRecord: function(store,model){
		var grid = this.getGridView();
		this.doOpenEditWindow('ui-core-PartStory-PartStoryContent-editwindow',grid,store,model,true);
	},
	doRemove: function(){
		var store = this.getGridStore();
		var grid = this.getGridView();
