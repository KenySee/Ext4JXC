Ext.define('Keer.ui.core.EntityEnum.MainContainer',{
	extend: 'Keer.widget.view.Container',
	alias: 'widget.ui-core-EntityEnum-maincontainer',
	controller: 'Keer.ui.core.EntityEnum.MainController',
	requires:[
	...
	initConfig: function (config) {
		Ext.apply(config,{
			gridStore: config.store || Ext.widget('EntityEnum-store',{
				controller:this.getController()
			}),
			navStore: Ext.widget('EntityEnum-store',{
				controller:this.getController()
			})
		});
	...
	initComponent: function(){
		Ext.apply(this,{
			...
			gridColumns:[
				...
				,{text:'名称',dataIndex:'name',width:220,editor:{xtype:'textfield'}}
				,{text:'备注',dataIndex:'remark',width:280,editor:{xtype:'textfield'}}
			],
			navColumns:[
				{text:'No.',xtype: 'rownumberer',width:32}
				,{text:'图标',dataIndex:'indexCls',width:40,xtype:'templatecolumn',align:'center',tpl:'<img alt="" src="data:image/gif;base64,R0lGODlhAQABAID/AMDAwAAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" class="x-action-col-icon x-action-col-0  {indexCls}">'}
				,{text:'枚举类型',dataIndex:'name',width:120,editor:{xtype:'textfield'}}
			],
			navpagingtoolbar : {
				xtype: 'pagingtoolbar',
				store: this.navStore,
