Ext.define('Keer.ui.core.PartReserve.MainController',{
	extend: 'Keer.widget.mvc.Controller',
	//【加载依赖】
	requires: [
		'Keer.ui.core.PartReserve.EditWindow'
	],
	//【混入功能】
	mixins: {
	},
	...
	doEdit: function(){
		...
		if (model){
			this.doOpenEditWindow('ui-core-PartReserve-editwindow',grid,store,model,false);
		}
		else {
	...
	doAddRecord: function(store,model){
		var grid = this.getGridView();
		this.doOpenEditWindow('ui-core-PartReserve-editwindow',grid,store,model,true);
	},
	doSave: function(){
		var store = this.getGridStore();
