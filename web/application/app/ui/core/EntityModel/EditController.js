Ext.define('Keer.ui.core.EntityModel.EditController',{
    ...
    control: {
    	...
    	layout: {
    		listeners:{
				change : function(editor, newValue, oldValue){
					...
					var field = form.down('#grid');
					var record = form.getRecord();
					if (newValue == 'form'){
						record.set('grid','Inline');
						field.setValue('Inline');
						field.disable();
					}
					else {
						var val = field.getValue();
						if (!val){
							record.set('grid','Popup');
							field.setValue('Popup');
						}
						field.enable();
					}
				}
    	...
    	parentModel: {
			listeners:{
				change : function(editor, newValue, oldValue){
					if (editor.valueModels && editor.valueModels.length > 0){
						var newValue = editor.valueModels[0];
						var form = editor.up('form');
						var record = form.getRecord();
						record.set('parentField',newValue.get('categoryField'));
					}
				}
			}
		},
			...
			listeners:{
				change : function(editor){
					if (editor.valueModels && editor.valueModels.length > 0){
						...
						var record = controller.get('record');
						record.set('categoryField',newValue.get('categoryField'));
					}
				}
			}
		},
	...
	onComboBeforeLoad: function(component,store,params){
		...
		if (component.itemId == 'parentModel' || component.itemId == 'categoryNav'){
			var record = this.get('record');
			Ext.apply(params,{
				clazzname: record.get('fullname')
			});
		}  		
  	},
	doCreate: function(){ 
