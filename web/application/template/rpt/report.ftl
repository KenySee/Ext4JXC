...
<title></title>
<OBJECT ID="_jatoolsPrinter" style='display:none' CLASSID="clsid:B43D3361-D075-4BE2-87FE-057188254255" codebase="jatoolsPrinter/jatoolsPrinter.cab#version=5,4,0,0"></OBJECT>
...
<body style='overflow:hidden;margin:0;padding:80px 0 5px 0;'>
...
<div id='header' >
	...
	<form method="post" action="ReportFilterAction!findList.action?file=${file}&viewer=${jspPath()}&datasource=${toDataSource()}>">
	<#assign itemlist = getWhereModels()>
	<#assign index = 0>
	<#list itemlist as item>
	${item.whereColumn.name}:
	<#if item.editorType == 'INPUT'>
	<s:textfield name="params.${item.whereColumn.code}"></s:textfield>
	</#if>
	<#if item.editorType == 'COMBOX'>
	<#assign index = index + 1>
	<s:select name="params.${item.whereColumn.code}" value="params.${item.whereColumn.code}" list="%{filterList${index}}" emptyOption="true"></s:select>
	</#if>
	</#list>
	<input type="submit" style='disabled:true' onclick =''   value="查询" size="12"></input>
	</form>
</div>
<% _container.writeOut(); %>
</jatools:report>
