Ext.define('Keer.ui.${upfolder}.${appfolder}.ChildController',{
	...
	control: {
		...
		toolbar_remove: {	listeners: { click: 'doRemove'	} }<#if grid?lower_case == 'popup'>,</#if>
		<#if grid?lower_case == 'popup'>
	...
	onBeforeLoad: function(store,params){
  	...
  	onCreateRecord: function(component,record){
     	...
     	var parent = this.get('parent');
		if (parent){
			record.set('${parentField}',this.getJSONBean(parent,'${parentModel.displayField}'));
		}
     	<#if layout?lower_case == 'listnav' || layout?lower_case == 'treenav' || layout?lower_case == 'gridnav'>
