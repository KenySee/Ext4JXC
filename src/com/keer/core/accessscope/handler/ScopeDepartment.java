...
@Description(Name="部门级")
@SuppressWarnings({ "rawtypes", "serial", "unchecked" })
public class ScopeDepartment extends AccessScopeHandler {
			...
			if (scope != null){
				for(Class<?> access : scope.value()){
					if (access == ScopeDepartment.class){
						...
						beanFilter += String.format("orgFullPath like '%s%s'", department.getOrgFullPath(),"%");
					}
				}
			}
			else {
					...
					if (scope != null){
						...
						for(Class<?> access : scope.value()){
							if (access == ScopeDepartment.class){
								...
								beanFilter += String.format("%s.orgFullPath like '%s%s'", prop,department.getOrgFullPath(),"%");
							}
						}
					}
				}
